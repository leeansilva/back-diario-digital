...
class Usuario(SQLModel, table = True):
    ...
    def Consultar(cls ,name : str, session : Session):
    	usuario = session.query(cls).filter(cls.nombre == name).all()
    	if usuario : return usuario
    	else : raise HTTPException(status_code=404, detail=f'No se encuentra el usuario en la base : ID {name}')

    @classmethod
    def Crear(cls,usuario : UsuarioCreacion, session : Session):
        usuario_db = session.query(cls).filter(cls.nombre == usuario.nombre).all()
        ...
        if (usuario_db == []):
            ...
            return nuevoUsuario
        else: return 'Ese nombre de usuario ya existe.'
    ...
    def Modificar(cls,name : str, usuarioModificado : UsuarioModificacion, session : Session):
        ...
        if usuario_db :
            ...
            return usuario_db
        else : raise HTTPException(status_code=404, detail=f'No se encuentra al usuario solicitado : ID {name}')
